...
import { Plus, Search, Download, Upload, MoreVertical, Edit, Trash2, Settings, UserCheck, UserX, AlertTriangle, Star } from "lucide-react"
...
export function ContactsList({ clientId, clientName }: ContactsListProps) {
  ...
  const handleExport = async (format: 'excel' | 'csv' = 'excel') => {
    try {
      ...
      a.href = url
      a.download = `contactos_${clientName}_${new Date().toISOString().split('T')[0]}.${format === 'excel' ? 'xlsx' : 'csv'}`
      ...
      toast({
        title: "Éxito",
        description: `Contactos de ${clientName} exportados en formato ${format.toUpperCase()}`,
      })
    } catch (error) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          <h2 className="text-xl font-bold tracking-tight">Contactos de {clientName}</h2>
          <p className="text-muted-foreground">
            Gestiona los contactos y leads específicos de este cliente
          </p>
        </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Lista de contactos para {clientName}
          </CardDescription>
        </CardHeader>
        <CardContent>
            ...
            <Table>
              ...
              <TableBody>
                {loading ? (
                  ...
                  <TableRow>
                    <TableCell colSpan={9} className="text-center py-8">
                      No se encontraron contactos para {clientName}
                    </TableCell>
                  </TableRow>
                ) : (
